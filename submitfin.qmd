...
Dimensions of dataset: 253680 x 22 Header/Footer data is displaying correctly All variable types are numerical. Summary function tells us dataset is well-maintained; no missing values, but BMI max is 98,which is questionable.
...
################
BMIC2<- ggplot(pred, aes(x = HDorAttack, y = BMI,fill=HDorAttack)) +
  geom_violin()+
  labs(x="Heart Disease Status",y="BMI",title="Violin map of BMI by Heart Disease Status")


BMIC3<- ggplot(pred, aes(x=HDorAttack,y=BMI,fill=HDorAttack))+
  geom_boxplot()+
  labs(x="Heart Disease Status",y="BMI",title="Boxplot of BMI by Heart Disease Status") + 
...
The barplot show how Age is distributed between Heart disease status. The heart disease increases with age showing that there might be an association between age and heart disease. The table below also shows that as age increases the proportion in each age categories who have heart disease increases.
...
ggplot(pred, aes(x = Income_cats, colour=HDorAttack,fill = HDorAttack)) +
  geom_bar(position = "dodge",stat="count") +
